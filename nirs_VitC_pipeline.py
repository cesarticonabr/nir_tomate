"""
Pipeline NIRS -> Vit. C (Vitamina C)
"""
...
t0 = time.time()
print("="*60)
print("NIRS -> Vit. C (Vitamina C) - PIPELINE")
print("="*60)
...
print(f'Vit. C: media={y.mean():.2f}, std={y.std():.2f}, [{y.min():.2f}, {y.max():.2f}]')

anova = f_oneway(*[dest[dest['Tratamento']==t][col_vitc] for t in sorted(dest['Tratamento'].unique())])
print(f'ANOVA Vit.C~Tratamento: F={anova.statistic:.3f}, p={anova.pvalue:.4f}')
...
for i in range(5):
    ...
    print(f'  PC{i+1}: var={pca.explained_variance_ratio_[i]:.2%}, r(Vit.C)={r:.4f}')
...
ax.set_xlabel('nm'); ax.set_ylabel('r'); ax.set_title(f'Correlacao Vit.C-espectro (max|r|={np.max(np.abs(corrs)):.3f})')
ax.grid(alpha=0.3)
...
plt.colorbar(sc,ax=ax,label='Vit. C'); ax.set_xlabel('PC1'); ax.set_ylabel('PC2')
ax.set_title(f'PCA (r(PC1,Vit.C)={pearsonr(X_pca2[:,0],y)[0]:.3f})'); ax.grid(alpha=0.3)
...
m1,m2=min(min(reals_p),min(preds_p))-2,max(max(reals_p),max(preds_p))+2
ax.plot([m1,m2],[m1,m2],'r--',alpha=0.7)
ax.set_xlabel('Vit. C Real'); ax.set_ylabel('Vit. C Predito')
ax.set_title(f'LOOCV (n=9) - Ridge - R2={r2_loo:.3f}'); ax.grid(alpha=0.3)
plt.tight_layout(); plt.savefig('analise_VitC_NIRS_final.png',dpi=150)
print('\nGrafico: analise_VitC_NIRS_final.png')
...
print(f'PCA: PC1 var={pca.explained_variance_ratio_[0]:.1%}, r(Vit.C)={pearsonr(X_pc[:,0],y)[0]:.3f}')
